...
const LeaveRequestsManagement: React.FC = () => {
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      const response = await apiClient.patch(`/leave-requests/${selectedRequest.id}`, updatePayload);
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmationModal
        ...
        message={
          actionType === 'approve'
            ? `¿Está seguro de que desea aprobar la solicitud de ${getEmployeeName(selectedRequest?.employee_id || '')} para ${translateRequestType(selectedRequest?.request_type || 'day_off')} del ${formatDateRange(selectedRequest?.start_date || '', selectedRequest?.end_date || '')}?`
            : (
              <div>
