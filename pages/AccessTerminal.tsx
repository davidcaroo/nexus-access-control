...
const AccessTerminal: React.FC = () => {
  ...
  const simulateScan = () => processAccess('101010'); // Simulates scanning a known employee for demo
          ...
          <div className="space-y-8">
            ...
            <Card className="bg-slate-800 border-slate-700 text-slate-100 min-h-[400px] flex flex-col justify-center relative overflow-hidden">
              {status.type === 'success' && status.employee ? (
                <div className="absolute inset-0 z-20 bg-emerald-500/20 backdrop-blur-md flex flex-col items-center justify-center text-center p-6"><div className="w-32 h-32 rounded-full border-4 border-emerald-500 overflow-hidden mb-4"><img src={status.employee.foto} alt={status.employee.nombre} className="w-full h-full object-cover" /></div><h2 className="text-3xl font-bold mb-1">{status.employee.nombre}</h2><p className="text-emerald-300 text-lg mb-6">{status.employee.cargo}</p><div className="flex items-center gap-2 bg-emerald-500/20 text-emerald-200 px-6 py-3 rounded-full text-xl font-bold"><CheckCircle2 size={28} /> {status.message}</div></div>
              ) : status.type === 'error' ? (
              ...
              {mode === 'scan' ? (
                ...
                <div className="p-8"><h3 className="text-xl font-semibold mb-6 text-center">Ingrese Cédula</h3><form onSubmit={handleManualSubmit} className="space-y-6"><input type="text" value={cedulaInput} onChange={e => setCedulaInput(e.target.value)} className="w-full bg-slate-900 border-2 border-slate-700 text-white text-3xl text-center py-4 rounded-xl" placeholder="000000" autoFocus /><div className="grid grid-cols-2 gap-4"><Button type="submit" isLoading={isProcessing} className="bg-emerald-600 hover:bg-emerald-700 py-4 text-lg"><LogIn className="mr-2" /> Entrada</Button><Button type="submit" isLoading={isProcessing} className="bg-amber-600 hover:bg-amber-700 py-4 text-lg"><LogOut className="mr-2" /> Salida</Button></div></form></div>
              )}
            </Card>
          </div>
